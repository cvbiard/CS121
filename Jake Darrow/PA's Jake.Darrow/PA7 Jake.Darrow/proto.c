...
void shuffle(struct card wDeck[13], struct card cards[52])
{
...
void deal(struct card wDeck[52], struct card hand[5], struct card dummy, int deckposition)
{
	...
	{	
		...
		if (wDeck[card].id >= 0)
		{
			hand[card - deckposition] = wDeck[card];
			printf("Card in hand at position %d has id %d, and is a %s of %s\n", card - deckposition, wDeck[card].id, wDeck[card].face, wDeck[card].suit);
			wDeck[card] = dummy;
		}
	}
	deckposition = card;
...
void frequency_function(struct hand p1, int frequency_table[13][2])
{
	for(int k = 0; k<13;k++)
		{
			for (int j = 0; j < 2; j++)
